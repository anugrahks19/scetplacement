...
function wrapFile(filePath) {
    ...
    if (filename.startsWith('auth-')) {
        console.log(`Skipping ${filePath} (auth page)`);
        return;
    }

    let content = fs.readFileSync(filePath, 'utf8');
    
    // Check if it already has the layout (e.g. if it has <aside class="navbar...)
...
// Process the targeted directories
const dirsToProcess = ['pages'];
